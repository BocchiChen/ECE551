...
board_t * makeBoard(int w, int h, int numMines) {
  //WRITE ME!
  board_t * bod = malloc(sizeof(*bod));
  bod->width = w;
  bod->height = h;
  bod->totalMines = numMines;
  bod->board = malloc(h*sizeof(*bod->board));
  for(int i = 0;i<h;i++){
    bod->board[i] = malloc(w*sizeof(**bod->board));
  }
  for(int y = 0;y<h;y++){
    for(int x = 0;x<w;x++){
      bod->board[y][x]=UNKNOWN;
    }
  }
...
int countMines(board_t * b, int x, int y) {
  ...
  for(int i = -1;i<=1;i++){
    for(int j = -1;j<=1;j++){
      ...
      if((y+j)>=0&&(y+j)<b->height&&(x+i)>=0&&(x+i)<b->width&&IS_MINE(b->board[y+j][x+i])){
        count++;
      }
    }
  }
